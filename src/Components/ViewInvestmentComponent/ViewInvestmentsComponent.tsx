...
export class ViewInvestmentsComponent extends Component<Investmentprops, Investmentstate>{
    ...
    componentDidMount() {
        axios.get<Investmentstate["investment"]>('https://localhost:8000/api/Investments').then(({data}:AxiosResponse<Investmentstate["investment"]>)=>{
            this.setState({
                investment:data
            })
        })
    ...
    clickHandler = (investmentId:number)=> {
        axios.delete<Investmentstate['investment']>(`https://localhost:8000/api/Investments/Delete/${investmentId}`).then(()=>{
